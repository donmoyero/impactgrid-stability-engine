...
/* ✅ UPGRADED: chat history is now actually used */
let aiChatHistory = [];
...
function updateProgressIndicator() {
  ...
  if (count < 3) {
    progress.innerHTML = `${count} / 3 months entered<br>Enter ${3 - count} more months to activate ImpactGrid Insights`;
  } else {
    progress.innerHTML = `${count} months recorded<br><strong>ImpactGrid Insights Activated</strong>`;
  }
}
...
function generateAIProjection(years) {
  ...
  /* ✅ UPGRADED: use average monthly growth instead of first-vs-last CAGR */
  const growthRates = [];
...
function renderAIInsights() {
  ...
  /* ✅ UPGRADED: anomaly detection surfaced in insights panel */
  const anomalies = ImpactGridAI.detectAnomalies(businessData);
  const anomalyHTML = anomalies.length > 0
    ? `<p><strong>⚠️ Anomalies:</strong> ${anomalies.map(a => a.date.toISOString().slice(0,7)).join(", ")} showed unusual revenue patterns.</p>`
    : "";
...
function renderPerformanceMatrix() {
  ...
  const growthScore = Math.max(0, growth * 4);
  const profitScore = Math.max(0, margin * 3);
  ...
  /* ✅ UPGRADED: health score clamped to 0–100 */
  const health = Math.min(100, Math.max(0, Math.round(
    (stability + growthScore + profitScore) / 3
  )));
...
function renderRiskAssessment() {
  ...
  if (volatility > 30) insight  = "Revenue volatility indicates unstable income patterns.";
  if (margin < 10)     insight += " Profit margin pressure detected.";
...
/* ✅ UPGRADED: async, uses chat history, typing indicator */
async function askImpactGridAI() {
  ...
  output.innerHTML += `<div class="ai-response" id="${typingId}">
    <span class="ai-typing">ImpactGrid AI is thinking<span class="dots">...</span></span>
  </div>`;
  output.scrollTop = output.scrollHeight;

  /* ✅ Call upgraded AI engine (async, with history) */
...
/* ✅ NEW: fills chat input from suggestion chips */

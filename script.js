...
function renderAIInsights(){
    ...
    setText("aiOperations",
    volatility>30
    ? "Operational data suggests revenue volatility that may affect income consistency."
    : "Operational revenue patterns appear relatively stable.");

    setText("aiForecast",
    growth>10
    ? "Forecast models suggest continued revenue expansion if current growth dynamics persist."
    : "Forecast modelling suggests moderate financial continuity based on current revenue trends.");

    const healthScore=Math.round((Math.max(0,100-volatility)+(Math.min(Math.abs(growth)*5,100))+(Math.min(margin*3,100)))/3);
